...
async def start_command(message: types.Message):
    await message.reply(
        f"Данный бот выдает погоду в любом городе.\n"
    ...
    try:
        if text == "/mycity":
            ...
            if cur.fetchone() is not None:
                cur.execute(
                    "SELECT city FROM cities WHERE id=?", (message.from_user.id,)
                )
                user_city = cur.fetchone()[0]
                await message.reply(
                    f"Используй /mycity *city*, чтоб записать город, который хочешь запомнить. "
            ...
            city_dicted = city_dict.get(
                text_without_command, text_without_command
            ).lower()
            r = requests.get(
            ...
            if data["cod"] == "404":
                ...
                cur.execute(
                    ...
                    (message.from_user.id, text_without_command),
                )
        ...
        cur.execute(
            ...
            (text_without_command, message.from_user.id),
        )
...
async def weather_command(message: types.Message):
    ...
    if text == "/weather" or text == "/weather@watislove_weather_bot":
        ...
        if cur.fetchone() is not None:
            ...
            await message.reply(
                f"Задай себе город через '/mycity *city*', где *city* - твой город"
            )
            return text
    command, text_without_command = text.split(None, maxsplit=1)
    ...
    try:
        ...
        if ((wind_degree >= 338) and (wind_degree <= 360)) or (
            (wind_degree > 0) and (wind_degree >= 22)
        ):
